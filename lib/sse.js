...
import { responseUsage } from "./stats.js";

export class SseTranslator {
  constructor(res, modelOrOptions = "deepseek-v4-pro") {
    const options = typeof modelOrOptions === "object" ? modelOrOptions : { model: modelOrOptions };
    this.res = res;
  ...
  feed(chunk) {
    ...
    const respUsage = responseUsage(usage);
    ...
    if (usage) {
      this.onComplete?.(usage);
      log.toks(usage.prompt_tokens, usage.completion_tokens, usage.total_tokens);
    }
